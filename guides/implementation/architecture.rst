...
Micetro Central listens on TCP port ``1231``. For more information, refer to :ref:`firewall-ports`.
...
.. [1] In smaller installations, Micetro Central can be installed on one of the DNS or DHCP servers, as it does not require many resources. More resources are needed as the managed environment gets larger.
...
Refer to :ref:`central-ha` for running multiple Central instances for high availability.
...
* In a Microsoft environment, the administrator can install the DHCP Agent on one, some, or all of the servers. If all the DHCP servers are in teh same security realm---maybe in different forests, but with trust between them---the DHCP Agent can be installed on one server, typically the server running Micetro Central.
...
* If some of the managed DHCP servers are not in the same forest as Micetro Central, and there is no trust between the forests, the administrator must install at least one DHCP Agent in the foreign forest. That DHCP Agent can act as a proxy between Central and the DHCP servers and must be running as a member of the AD DHCP Administrators group in the foreign forest.
...
Cisco IOS DHCP servers can be managed using Micetro. A Micetro DHCP Agent must be installed on a machine in the environment, which acts as a proxy to manage the Cisco IOS DHCP servers and uses either plain ``telnet`` or ``ssh`` to connect to the managed servers.
