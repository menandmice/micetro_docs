...
=========================================
Micetro supports agent-free management of DNS and DHCP servers for some servers. These include Microsoft DNS/DHCP and ISC Kea DHCP.

.. _kea-agent-free:

Kea DHCP
--------
Kea DHCP servers can be configured agent-free, without a DHCP Agent running on every Kea machine. The :ref:`kea-control-agent` must be installed and available for communication through its defined port (default: ``8000``). A single DHCP Agent is sufficient to communicate with and handle all Kea servers on the network.

.. _ms-agent-free:

Microsoft DNS/DHCP
------------------
Microsoft DNS and DHCP servers in Active Directory (AD) environments can be managed agent-free, i.e., without running a Micetro agent locally on the DNS/DHCP server.
...
If the DNS/DHCP servers are distributed over a wide geographic area, you may experience better performance if you install one agent in each geographic location. For example, if there are data centers in Iceland, India, and the United States, it is probably best to install one proxy agent in each data center that each handles communications with the DNS/DHCP servers in its data center. The proxy agent, in turn, feeds all the information back to Micetro Central, eventually reaching the end-user in one of the Micetro user interfaces.
...
The same rule applies to DHCP server management. If the service account has read-only privileges to the scopes, the scopes will be displayed in Micetro but the end-user will not be able to make any changes.
...
DNS:
""""
With agent-free management of MS DNS servers, as opposed to using a locally installed agent, you lose the following abilities in static zones only:
