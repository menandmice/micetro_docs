...
.. code-block:: bash
  ...
  tar -xzvf mmsuite-controllers-9.3.9.linux.x64.tgz

In the newly created ``mmsuite-controllers-9.3.9.linux.x64`` directory run the installer script to install the Men&Mice Controller (as root):

.. code-block:: bash

  cd mmsuite-controllers-9.3.9.linux.x64 && ./install --skip-arrange
