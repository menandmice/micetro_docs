...
Run the downloaded binary and follow the on-screen instructions.
...
|
.. important::
    The installer creates the data directories for Micetro Central at the default location ``C:\Program Data\Men and Mice\Central`` even if the installation target is on a different drive or path.
