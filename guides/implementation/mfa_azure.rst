...
Register the Application
   ...
   3. Enter the name and select the appropriate authentication types. For the Redirect URI, select :guilabel::`web` and enter https://micetro.central.fqdn/mmws/auth_cb/microsoft. 
...
4.	Select :guilabel:`Configure` and choose :guilabel:`Azure AD` in the dropdown.
5.	Complete the configuration form with the information collected during the Entra ID setup process.
    ...
    * **Scope** (optional): Defines the level of access that the client application requests from the user during the authentication process.
...
------------------------------------
Instead of using the Micetro Web Application (see above), it's possible to configure external authentication manually by creating a JSON configuration file in Micetro Central's data directory. Upon startup, the Micetro Central program will search the data directory for a file named ``ext_auth_conf.json``.
...
For more information on manually configuring external authentication, refer to the documentation for version 10.3. You can find detailed instructions in the section titled :ref:`Multi-factor-Authentication`.
