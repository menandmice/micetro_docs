...
By default, the installer attempts to automatically detect the installed DNS service (e.g., BIND) and install the appropriate agent. If it can't detect the service, it provides hints and additional information.
...
For machines with multiple services (for example, ISC DHCP and ISC BIND DNS), explicitly specify the desired agents during installation.
...
To view available agent options and parameters, run the installer script with the --help parameter:
  ...
  .. code-block:: bash
    ...
    Micetro server agent installer.
    --help:  Print help.
    --quiet:  Suppress output during install.
    --auto:  Automatically determine what agents to install. Default if no other option is given.
    --bind-dns-controller:  Install a DNS server agent for BIND.
    --unbound-dns-controller:  Install a DNS server agent for Unbound.
    --generic-dns-controller:  Install a Generic DNS server agent.
    --isc-dhcp-controller:  Install a DHCP server agent for ISC dhcpd.
    --kea-dhcp-controller:  Install a DHCP server agent for Kea dhcp4.
...
If you plan to use the Generic DNS agent, refer to the :ref:`generic-dns-controller` for more details.
...
During the installation process, the installer will prompt you with questions related to the Micetro DNS Server agent. Be prepared to answer the following:

  * Do you want to install the Micetro DNS Server agent?
  * Are you running named in a chroot() environment?
  * What is the chroot() directory?
  * Where is the BIND configuration file?
  * Would you like the DNS Server agent to run ``name-checkconf`` to verify changes when editing advanced server and zone options?
  ...
  * Enter the user and group names under which you want to run the Micetro DNS Server agent. This must be the user which is running named.
  * Where would you like to install the Micetro external static zone handling utilities?
  * Where do you want to install the Micetro Server agent binaries?
...
The Micetro DNS Server agent requires specific settings within the ``named.conf`` file (including any files listed in ``include`` statements in ``named.conf``). Ensure the following statements are present:
...
* ``controls``: The Micetro DNS Server agent uses a ``controls`` statement for BIND. There must be a ``controls`` statement with an ``inet`` substatement that references an explicitly defined key (as mentioned above). The ``inet`` statement should allow connections from the loopback address, ``127.0.0.1``. If no ``controls`` statement is defined, the installer will prompt you to create one manually.

Changes in named.conf
^^^^^^^^^^^^^^^^^^^^^

Note that the installation of the Micetro DNS Server agent will rearrange your named configuration data, including rewriting ``named.conf`` and reorganizing the data directory. The new configuration is functionally equivalent to the old one, except that the logging statement may be added or modified to include new channels.
...
.. csv-table::
  ...
  "Micetro DNS Server agent daemon", "mmremoted, usually in /usr/sbin or /usr/local/sbin"
  "Micetro external static zone handling utilities", "mmedit and mmlock, usually in /usr/bin or /usr/local/bin"
  "Data directory for Micetro DNS Server agent", "Usually /var/named, /etc/namedb, /var/lib/named, or something within a chroot jail; the same location as before the DNS Server agent was installed"
...
Removing the DNS Server agent and Reverting to Original Data
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Stopping the Service
""""""""""""""""""""
Use the init script to stop the DNS Server agent service. You can achieve this by providing the *stop* argument to the init script. For example:
...
"""""""""""""""""""""""""
Once the service is stopped, you can proceed to remove the DNS Server agent files:

   * Delete the daemon binary file associated with the DNS Server agent.
   * Delete the init script used to start the DNS Server agent service.
...
1. Stop the BIND or named service, which might have been managed by the DNS Server agent, using its respective init script. For example:
...
2. With the BIND or named service stopped, you can proceed to restore your original DNS configuration and data:
   * Delete the initial configuration file (``named.conf``) created by the DNS Server agent. 
   * Delete the data directory created by the DNS Server agent.
...
These steps will effectively remove the DNS Server agent and revert your DNS setup to its original state. Be cautious when performing these actions, as they may impact your DNS service.
...
After installing the DNS Server agent, run the following commands as root:
...
Verify the DNS Server agent is running
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Micetrol Server agent running on Windows
---------------------------------------------
...
In some cases, Micetro DNS Server agent will also need to be told specifically which interface to use when requesting zone transfers. If you have trouble opening a dynamic zone after setting the zone's transfer restrictions appropriately, check the Event Log / Application Log for messages from Micetro DNS Server agent. If there is a message indicating that it was unable to get a zone transfer, note the address it tried to use; you can either add that IP address to the transfer restrictions list, or else edit a configuration file for Micetro DNS Server agent.

To configure the DNS Server agent to use a different address, edit the service's preferences.cfg file on the DNS server computer. The file is located in one of the following two locations, where {Windows} is probably C:\\Windows:
...
Running Micetro DNS Server agent under a privileged user account / Server type: "Microsoft Server Controller-Free"
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Normally, the Micetro DNS Server agent is installed on only *one* host in an Active Directory forest, or one copy per site. That installation can then manage all MS DNS servers in the forest, or in the site, using Microsoft's own DNS management API. In order for this to work, the service needs to run as a user that has DNS management privileges (i.e. the AD service account must be a member of the DNSAdmins group of the domain).

To configure Micetro DNS Server agent to access DNS servers on remote computers, do the following:

 1. Start the Windows 'Services' program and open the properties dialog box for Micetro DNS Server agent.
 ...
 4. Close the dialog box and restart the Micetro DNS Server agent service.

If Micetro DNS Server agent is run as a local system service (the default), it will only be able to manage the MS DNS service on the same host.

Enable the Generic DNS Server agent functionality
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Configure the DNS Server agent to work with Microsoft Azure DNS
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

For information on configuring Microsoft Azure DNS, see :ref:`configure-azure-dns`.

Where to install Micetro DNS Server Agent
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If Men&Mice Central is installed on a Windows host, one option is to install Micetro DNS Server agent on the same host. If this is not done, the system will need to be told where to find the DNS Server agent when adding a new DNS server to the system. This will be presented as connecting via proxy.

.. note::
  The Micetro communication protocol used to control a DNS server is more efficient than the Microsoft protocol. This means that if a DNS server is separated from Men&Mice Central by a slow network link, it is more efficient to install a copy of the Micetro DNS Server agent in the same local network (the same site, typically) as the DNS server.
