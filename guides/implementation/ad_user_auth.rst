...
------------------------------------------------------------------
Even with AD user authentication enabled, you must still create users and assign privileges to them in Micetro. The only difference between AD and local user authentication is in the authentication process: when AD user authentication is enabled, users are authenticated via the AD user authentication system before accessing Micetro. With AD user authentication, user passwords are not stored within Micetro.
...
1. Navigate to :menuselection:`Admin --> Configuration` and select :guilabel:`Users` in the left sidebar.

2. To add a new user, select :guilabel:`Create` in the task bar. Enter the user information into the dialog, selecting :guilabel:`Active Directory` from the **Authentication type** dropdown. For more information about how to create users, refer to :ref:`acl-users`.
   
  .. note::
    If Micetro Central is not running on a Windows machine, only the Micetro authentication method will be available.

3. To modify an existing user, double-click the user's name to display the **Edit Properties** dialog and select :guilabel:`Active Directory` from the **Authentication type** dropdown.
...
2. In the left sidebar, select :guilabel:`Authentication` under :guilabel:`System Settings`.
...
2. In the left sidebar, select :guilabel:`Groups` under :guilabel:`Access Management`.

3. In the data grid, double-click the group for which you want to configure AD. If the desired group is not listed, you'll need to add it. For more information about how to create groups, refer to :ref:`acl-groups`.
...
.. note::
  Group-level AD user authentication is only supported when Micetro Central is running on a Windows machine that is a member of an Active Directory domain or forest.
