...
.. note::
    For the management of the DHCP failover in Windows Server 2012 R2, the service account must also be a member of the local Administrators group of the DHCP servers to be able to fetch/manage the failover configuration.

To configure the Micetro DHCP Agent to access DHCP servers on remote computers:

  1. Start the Windows **Services** program and open the **Properties** dialog box for the Micetro DHCP Agent.
