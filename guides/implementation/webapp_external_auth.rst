...
Integration with Azure Active Directory and Okta enables seamless MFA and single sign-on (SSO) capabilities.
After configuration, Micetro's front page will feature buttons directing users to the respective provider's authentication URL.
...
.. Note::
   If running Central in HA mode, it is recommended to disable the service on one of the partners. This will help ensure that the installation is successful on each server and prevent the servers from failing over during the installation process.
...
**Register the Application**
   ...
   2. On the left pane, select :guilabel:`App registration` and then click :guilabel:`New Registration` within the newly opened “blade” (what Azure calls their subpages).
