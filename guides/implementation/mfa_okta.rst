...
.. Note::
   If running Central in High Availability (HA) mode, it's recommended to disable the service on one of the partners. This helps make sure that the installation is successful on each server and prevent the servers from failing over during the installation process.
...
Installation/setup
-------------------
To begin the configuration process, you need to set up an application within Okta. This step provides you with the necessary properties required for configuration.
...
4.	Select :guilabel:`Configure` and choose :guilabel:`Okta` in the dropdown.
...
------------------------------------
Instead of using the Micetro Web Application (see above), it's possible to manually configure external authentication by creating a JSON configuration file in Central's data directory. 
...
For more information on configuring external authentication manually, refer to the documentation for version 10.3. You can find detailed instructions in the section titled :ref:`Multi-factor-Authentication`.
